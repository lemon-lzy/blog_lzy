...
###  方法区
方法区是一个抽象，不同版本又两种实现：
- jdk1.7中, 方法区的实现是**永久代**, 而永久代是**在堆中**的. 方法区存储了类信息Class, 类加载器ClassLoader, 运行时常量池. 静态变量。运行时常量池中存储了字符串常量池String Table
...
- java8中, 方法区的实现是元空间, 元空间在本地内存(操作系统内存)中. 方法区同样存储了类的元信息,  静态变量，运行时常量池. 而字符串常量池String Table 被放在了堆区
...
# 什么是java垃圾回收机制？
JVM 垃圾回收机制主要针对堆内存进行管理。
## 判断对象是否可回收使用可达性分析算法：
以 GC Roots 为起点向下搜索，能被引用链关联到的对象为存活对象，不可达的对象判定为垃圾。GC Roots 主要包括：
- 虚拟机栈中引用的对象
- 本地方法栈引用的对象
- 方法区中静态变量、常量引用的对象
## 常见垃圾回收算法：
## 标记 - 清除
先标记存活对象，再清除垃圾对象。优点简单，缺点是会产生大量内存碎片。
### 标记 - 复制
将内存分为两块，每次只使用一块，回收时将存活对象复制到另一块，清空原区域。无碎片，但空间利用率低，适合存活对象少的场景。
### 标记 - 整理
标记存活对象后，将存活对象向一端移动，清理边界外的内存。无碎片、空间利用率高，但移动对象有开销。
## 常见垃圾回收器：
- **Serial**：单线程收集器，适用于客户端。
- **Parallel**：多线程并行收集，注重吞吐量。
- **CMS**：并发标记清除，以低停顿为目标，会产生内存碎片。
- **G1**：分区回收，可预测停顿，JDK9 默认。
- **ZGC / Shenandoah**：超低延迟收集器，停顿极短，适合对延迟敏感的服务。
### 选择上：
- 追求吞吐量用 Parallel
- 追求低延迟用 G1、ZGC
## 二、三种算法 和 分代收集 的关系（面试必问）
### 什么是分代收集？
JVM 堆分为： 
- 新生代：对象朝生夕死，存活率低
- 老年代：对象存活时间长，存活率高
- 分代收集 = 根据不同代的对象特点，选择不同的垃圾回收算法。 
### 对应关系
#### 新生代 → 用 标记 - 复制算法
- 新生代对象死得快，存活少
- 复制算法效率高、无碎片
- 所以新生代用复制算法（Eden、Survivor 复制移动）
#### 老年代 → 用 标记 - 清除 或 标记 - 整理
- 老年代存活率高，复制成本太大
- 所以不用复制算法
#### 回收器和回收算法
- CMS 用标记 - 清除（低停顿，但有碎片）
- Serial Old、Parallel Old 用标记 - 整理（无碎片，更稳定）
...
CMS是针对老年代的一个并行垃圾收集器, 采取**标记清除**算法, 目的是为了追求**低停顿**时间,

那他是怎么实现这个低停顿的呢，那就要说一下这个CMS垃圾回收器的一个处理过程了:
## CMS 处理过程有四个步骤：
1. **初始标记**：指的是标记所有被 GCRoots 直接可达的老年代对象，该阶段**需要STW**(标记对象很少，符合低停顿，可以STW)。
2. **并发标记**：指的是对**初始标记**的对象继续**递归标记**出所有存活老年代对象，该阶段与用户线程同时运行, **不需要STW**(因为遍历的对象太多为了低停顿)
3. **重新标记**：修正**并发标记期间**因为用户程序继续运行而导致浮动垃圾和漏标问题，该阶段**需要STW**. (由于垃圾回收算法和用户线程并发执行，虽然能降低响应时间，但是会发生漏标和多标的问题,需要STW,不然清理不干净)。
4. **并发清除**：指的是将**标记为垃圾的对象**进行清除，该阶段与用户线程同时运行, **不需要STW**(追求低停顿，所以在清理时不需要STW).
