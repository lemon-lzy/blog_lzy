...
## JRE
- JRE是**Java运行时环境**，是Java程序运行所需的最小环境。它包含了JVM和一组Java核心类库(比如java.lang通过Integer等，java.utils提供ArrayList等)，用于支持Java程序的执行.
- 简单来说: **JRE=JVM+一组核心类库**
## JDK
- JDK是**Java开发工具包**。它包含了**JRE**、编译器（javac)、java、jar等开发工具。
...
### double会出现的问题：
- float和double是用二进制来表示浮点数, 所以做浮点数计算时会出现精度丢失问题，像 0.1 这种十进制小数在二进制里是无限循环小数，存储时只能截断，精度就丢了。看个经典例子：
````
...
- 占用的内存不同。相比于对象类型需要额外存对象头， 基本数据类型占用的空间非常小。
...
- 默认情况下，这个范围是-128至127。当创建一个在这个范围内的整数对象时，会复用缓存中的现有对象，而不是新建一个对象。所以, 在对比是一定要用equals().
...
# 深拷贝和浅拷贝区别？什么是引用拷贝？
![img.png](img.png)
- **深拷贝**:深拷贝拷贝出来的对象是一个完全独立的副本。
- **浅拷贝**:浅拷贝对于基本数据类型赋值的是值的副本，引用类型复制的是地址，也就是两者的引用类型指向同一块内存，对拷贝对象进行操作会影响原对象。
